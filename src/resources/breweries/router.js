...
breweriesRouter.get("/:id", (req, res) => {
    ...
    if(foundBrewery) {
        ...
        res.json({msg: `Sorry, no breweries with id: ${breweryId}`})
    }
});
breweriesRouter.post("/", (req, res) => {
    ...
    res.json({newBrewery: newBrewery})
})
breweriesRouter.delete("/:id", (req, res) => {
    const breweryToDeleteId = Number(req.params.id)

    const breweryToDelete = breweries.find(brewery => brewery.id === breweryToDeleteId)
    if(breweryToDelete) {
       const newBreweries = breweries.map(brewery => {
           if(brewery.id === breweryToDeleteId) {
               return 
           }else {
               return brewery
           }
